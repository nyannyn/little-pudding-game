...
import { makeWorld, runOneBath } from './helpers';
...
describe('D25 xp 來源', () => {
  ...
  it('撿、加工、賣各給對應的 xp', () => {
    ...
    s.drops.push({ id: 'd2', zone: START_ZONE, species: 'caramel', pos: { x: 0, z: 0 }, bornAt: 0 });
    pickAllDrops(s, sink);
  ...
  it('自動化做的也算 xp（裝了設備等級不會停）', () => {
    ...
    expect(runOneBath(w, 'caramel')).toBe(true);
    expect(w.state.xp).toBe(BALANCE.xp.bath + BALANCE.xp.pick);
  });
});
