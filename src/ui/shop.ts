...
import { ART, INGREDIENT_ART, artFor, type ArtSpec } from './shopArt';
...
function sellCardHtml(s: SpeciesId): string {
  ...
  return `<div class="card" data-id="sell:${s}" data-status="available">
    ...
    <div class="desc">加工成${info.dessert}可賣 ${dessertPrice(s, BALANCE.dessertPriceMult)}／份</div>
  ...
  render(state: GameState) {
    ...
    if (key !== this.structureKey) {
      ...
      this.body.innerHTML =
        this.page === 'sell'
          ? sellable.length
            ? `<div class="grid">${sellable.map(sellCardHtml).join('')}</div>`
            : '<div class="empty">還沒有原料。讓布丁泡個澡，泡完會掉在盆邊。</div>'
