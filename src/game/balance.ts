...
export const BALANCE = {
  ...
  /**
   * 地板上最多堆幾份掉落物，滿了就不再掉。
   * **2026-09-22 由 5 調到 8**：D32 之後地上同時有蛋與物種原料兩種東西，
   * 5 格很容易被蛋佔滿——佔滿就完全停產，玩家看到的是「布丁不動了」。
   * 實際踩過：e2e 連掉 5 顆蛋（機率 0.65^5＝11.6%，固定 seed 必然重現）就卡死。
   * 掉落物是 InstancedMesh，加格子不會多 draw call。
   */
  dropCap: 8,
  /**
   * 原料掉在離澡盆多遠的地方（世界單位）。
  ...
  // ── 產出與配方（D32／D33，2026-09-22 改版）──────────
  /**
   * 每隻布丁每隔幾秒掉一份原料（實際間隔會在 ±`dropJitter` 之間抖動，
   * 否則整層的布丁會同步掉、看起來像機器）。**不看 caramel**（D32）。
   * 一份甜點要 2 蛋 ＋ 1 物種原料＝約三次掉落，20 秒一份時一隻布丁約 60 秒供得起一份甜點，
   * 跟改版前「一輪泡澡 65 秒產一份原料」同量級，節奏不會整個跑掉。
   */
  dropIntervalSec: 28,
  dropJitter: 0.25,
  /**
   * 低於這個焦糖值就停止掉落（D35）。
   * 使用者原話是「布丁保持愉快、時間到了自然產生」——這是那個「愉快」的最小判定：
   * 焦糖見底的布丁不生產。液體用完 → 泡不了澡 → 焦糖歸零 → 停產，
   * 焦糖澡與補貨因此重新有了機械意義（D32 之後它一度變成純裝飾）。
   */
  dropCaramelMin: 1,
  ...
  /**
   * 泡牛奶澡時把配子推向鮮奶酪的機率（D34）。
   * 牛奶＝鮮奶酪的風味來源：不這樣接，單親複製＋拿掉變白之後鮮奶酪就永遠養不出來。
   */
  ...
  // ── 星級布丁（D62–D65，2026-09-25）──────────────────
  /**
   * 升下一星要的照顧點數：`starCare[i]` 是 ★(i+1) → ★(i+2)。
   * 實測（2026-09-25）精養區 5 隻共用一盆、泡本命液，每隻每小時約 150 點。
   * 世代鏈（D63）要一代一代從 ★1 養起，養到 ★5 的點數＝4·c1＋3·c2＋2·c3＋c4。
   * **月玩家量表校準（AC11-12）**：計畫估月玩家一天 8.75 遊戲小時，實際是約 21（三段離線 5h＋8h＋8h 上限），
   * 而且換代只在上線時發生、每一代多等半天。計畫給的 [360, 1080, 2880, 7200] 量到 ★3 第 3 天（太慢）、★5 第 9 天（太快）；
   * 前兩級降、最後一級拉高之後落在目標內（數字見計畫 CP11 的校準紀錄）。計畫更早的第一版（30／90／240／600）第 1–2 天就出 ★5，已作廢。
   */
  starCare: [240, 720, 2880, 16000] as readonly number[],
  /** 泡一次澡給幾點；泡的是本命液（物種等位基因對應的液體）時給 `careNative` 點（D62） */
  careBath: 1,
  careNative: 3,
  /** 開局與舊存檔的布丁最高能長到幾星（D63） */
  startPotential: 2 as 1 | 2 | 3 | 4 | 5,
  /** 精養區最多住幾隻（D62） */
  eliteCapacity: 5,
  /** 星級價值倍率（D65）：索引 0＝★1。甜點售價、直接賣原料都乘這個 */
  starMult: [1, 1.5, 2.2, 3.2, 5] as readonly number[],
  /** 散客最多付到幾星的倍率（D65） */
  walkInStarCap: 2 as 1 | 2 | 3 | 4 | 5,
