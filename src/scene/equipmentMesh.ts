...
/** 販賣機的 x（區域座標）；為什麼是這個數字見 `vendingMachine` */
const SELLER_X = -0.14;
 ...
 * 會直接卡在機身裡。
 */
function vendingMachine(parts: Parts, floorY: number, d: number) {
  // 正面偏左一點點：右邊 x≥0.045 是該層名牌（cabinet.createPlates）會蓋到的區域，
  // 左邊 x≤−0.22 是澡盆（BASIN_SLOTS[0]），機身再往左會把澡盆整個擋在後面。
  const x = SELLER_X;
  const front = d / 2 - 0.04;     // 機身正面
 ...
 */
export const SELLER_SPOUT = { x: SELLER_X - 0.03, y: 0.66, z: TANK.depth / 2 - 0.09 } as const;
 ...
 */
function build(id: EquipmentId, parts: Parts, floorY: number, ceilY: number, basin: { x: number; z: number }) {
  ...
  switch (id) {
    ...
    case 'collector': {
      // 頂板的滑軌＋垂下來的夾爪
      box(parts, METAL, w * 0.7, 0.035, 0.05, 0, ceilY - 0.05, -d * 0.12);
      cyl(parts, METAL, 0.014, 0.18, -w * 0.12, ceilY - 0.14, -d * 0.12);
      box(parts, ACCENT, 0.075, 0.05, 0.075, -w * 0.12, ceilY - 0.24, -d * 0.12);
      box(parts, ACCENT, 0.022, 0.06, 0.022, -w * 0.12 - 0.03, ceilY - 0.29, -d * 0.12);
      box(parts, ACCENT, 0.022, 0.06, 0.022, -w * 0.12 + 0.03, ceilY - 0.29, -d * 0.12);
      break;
    }
    case 'crafter': {
...
export class EquipmentView {
  ...
  sync(state: GameState, zone: string, ox: number, floorY: number, ceilY: number) {
    ...
    const parts: Parts = [];
    for (const id of owned.split(',') as EquipmentId[]) build(id, parts, floorY, ceilY, basin);
