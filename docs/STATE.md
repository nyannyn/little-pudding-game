...
- 證據：單元 168 綠、e2e 48/48 綠＋新 `cp7-instancing.spec.ts` 3 條（體色寫死成白 → 紅過；眼睛 instance 抄成本體矩陣 → 紅過。第三條讀的是 `Puddings_Eyes.instanceMatrix` 不是骨架——`cp3-pudding-look` 只量骨架，pool 抄錯矩陣它照樣綠；比值要除掉同一隻本體的比值，跳躍時根節點自己會拉長 1.19）、`npm run build` 綠、`npm run models:optimize` ok（1228 面／23,968 bytes）、截圖前後並排（本體／焦糖／眼睛／腮紅一致）、六物種顏色各異、泡澡閉眼。**視覺簽核被退回（不是通過）**：使用者要「不同口味不同建模」→ D44（兩種輪廓＋五種加件，最壞 23 draw calls）；狀態卡改表情＋頭頂小圖示 → D45；順序定為 7-2a → 7-1b／7-2b → 7-3。
