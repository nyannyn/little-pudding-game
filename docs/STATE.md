...
- **D61**：機器 Lv1–20（每級秒數 ×0.96、上限＋1、失敗 ×0.7；每 5 級一階鐵→銅→銀→金，星星一階 5 顆、顏色＝階級，底座牌左邊色帶同色，跨階 toast）。店面人氣 Lv1–20（客人間隔每級 ×0.93；Lv6 店員自動上架＝離線也賣、Lv11 一次最多買 3、Lv16 最多 4），商店工坊頁第八張卡（`fame.svg` 自己畫）。**升級價七台與人氣同價**：60 × 1300·N³／(N³＋125)。`SCHEMA_VERSION` 9→**10**：機器 1→1、2→3、3→5（`V9_MACHINE_LEVEL`，每一項都不比舊的差）、人氣補 1、站上補 `startedAt`；分辨新舊看有沒有 `fame` 欄。
...
- **證據**：vitest 265 綠（新 `tests/unit/batchLevels.test.ts` 17 條）；`npm run test:negative` 新增 AC10-1／2a／2b／3／3b／4a／4b／5a／5b 全紅過（AC10-5b 第一版**沒紅**：測試比的是賣掉份數，Lv11 起客人多買把「間隔不吃等級」蓋過去，改成數客人才紅），只剩 master 上本來就壞的 AC2-9；e2e **86 passed**（新 `cp10-batch.spec.ts` 3 條：疊份數整套、七站×20 份畫 83 杯＋draw calls ≤ 35、出爐字卡兩盤合一）；`npm run build` 綠。
- **待使用者**：畫面簽核（線上網址）；3 小時量表裡升級變慢（Lv2 約 50 分鐘，改前 11 分；3 小時做出的甜點 600–900→340–400 份）是「同等困難」的直接代價，要先講。已知下一道牆：農場約 3 小時養滿 60 隻，月玩家的收入第 4 天起持平在 23–32 萬／天——長線靠的是離線累積＋機器／人氣兩條升級，農場本身沒有長線。
