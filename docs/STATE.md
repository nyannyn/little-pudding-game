...
- **證據**：單元 198 綠（新 `furniture.test.ts` 15 條；突變紅過：migrate 不特判倉庫盆、拿掉布丁避家具）；e2e 新 `cp7-furniture.spec.ts` 4 條（突變紅過：拖曳時不關 controls、放手不寫入、× 不寫 localStorage）；`npm run build` 綠；pacing 四情境里程碑都在原範圍（最後一個情境 10.3／19.4／33.9 → 10.8／17.4／33.4 分，亂數序列因避障改變）；截圖兩個視口（拖曳中、倉庫卡武裝狀態）看過。**視覺待使用者手機簽核**：長按手感（450ms）、綠紅圈、倉庫卡。
