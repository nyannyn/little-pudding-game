...
**使用者原話**：「甜點店的機器升級 應該縮短製作時間跟增加一次製作的量」「我希望能夠讓玩家有一次做一批甜點的感覺」；追問後：「一開始決定要製作的甜點 同樣份量應該要可以疊加」「每次升級應該都要同級距且同等困難」「你應該再想的更長遠 規劃一個可以玩一個月的遊戲」「這個遊戲要長久遊玩 應該要有多個等級去攻克 而且要吸引人」。看過提案選「照做，A＋B 一起上線」。

- **開工前量到的前提**（`npm run pacing` 加探針）：整條線一小時就升滿 Lv3（每盤平均 3.2–4 份，貼著上限 4）；**需求才是天花板**——Lv3 之後每 20 分鐘做約 40 份、客人只買約 25 份、積壓 3→81 份、3 小時 3.6 萬花不完。所以只拉長機器等級沒有意義，一定要加需求那條（店面人氣）。
- **D60**：菜單點卡片＝＋1 份（`hud.ts` 的 `draft`，不進 `GameState`）、「－」「最多」「開始製作 ×N」；數字**就地改**不重建卡片（連點時手指底下的節點不能被換掉）。`startBatch(state, species, qty, emit)` 超過 `maxBatch` 就拒絕、不夾。每站秒數看**自己**的等級，進站那一刻定 `startedAt`／`doneAt`，中途升級不影響那一盤（進度條用 doneAt−startedAt）。畫面：攪拌碗隨份數變大、裝模後幾份畫幾杯（1／2／3 排、排不下縮小，`MAX_ITEMS` 64→256、滿了 console.error）、出爐一杯接一杯跳進成品櫃、「出爐！×N」字卡（同一幀多盤合一張，microtask 累加）。
- **D61**：機器 Lv1–20（每級秒數 ×0.96、上限＋1、失敗 ×0.7；每 5 級一階鐵→銅→銀→金，星星一階 5 顆、顏色＝階級，底座牌左邊色帶同色，跨階 toast，機身本體不換色）。店面人氣 Lv1–20（客人間隔每級 ×0.93；Lv6 店員自動上架＝離線也賣、Lv11 一次最多買 3、Lv16 最多 4），商店工坊頁第八張卡（`fame.svg` 自己畫）。**升級價七台與人氣同價**：60 × 1300·N³／(N³＋125)。`SCHEMA_VERSION` 9→**10**：機器 1→1、2→3、3→5（`V9_MACHINE_LEVEL`，每一項都不比舊的差）、人氣補 1、站上補 `startedAt`；分辨新舊看有沒有 `fame` 欄。
- **校準**：月玩家量表（`PACING_ONLY=month npm run pacing`，每天 3×15 分鐘、30 天、離線照 8 小時上限）。五種價錢規則的結果表在計畫檔 CP10；最後這組每 5 天 37／23／20／19／19／20、第 30 天 Lv18–19（兩個 seed 都過）。**教訓**：各台各一組價錢時最貴那台會被一路留到最後；bot 自己卡自己（補貨太少、開場把離線存貨賣光）會讓量到的是 bot 不是平衡——兩個都修了。
- **證據**：vitest 265 綠（新 `tests/unit/batchLevels.test.ts` 17 條）；`npm run test:negative` 新增 AC10-1／2a／2b／3／3b／4a／4b／5a／5b 全紅過（AC10-5b 第一版**沒紅**：測試比的是賣掉份數，Lv11 起客人多買把「間隔不吃等級」蓋過去，改成數客人才紅），只剩 master 上本來就壞的 AC2-9；e2e **88 passed**（新 `cp10-batch.spec.ts` 5 條：疊份數整套、七站×20 份畫 83 杯＋draw calls ≤ 35、出爐字卡兩盤合一、v9 存檔走「還原」、Lv10 以上底座牌不溢出；e2e 負向對照三組紅過：草稿兩層上限都拿掉、杯數夾回 4、兩位數排版關掉）；`npm run smoke:live` 對本機正式建置全過；`npm run build` 綠。
- **面數（CLAUDE.md「總面數 ≤ 50k」沒有自動檢查，量了）**：甜點店七台全買、**改前**最壞情況（四站各 4 杯＋成品櫃＋展示架）就已經 **87,456**，光機器沒杯子約 48k——50k 在甜點店本來就超標，不是這次造成。這次一盤 20 份原本會衝到約 14 萬（每杯頂上的奶油頂約 980 面、還沒裝飾的杯子也在畫一個縮成 0 的頂），改成**只畫有頂的**＋奶油頂段數 8×20→6×14：同情境改後 64,160、七站各 20 份滿載 70,304，都比改前少；draw calls 19。
- **3 小時量表**：第一版有兩項慢超過 25%（equip-first 整條線買齊、zone-first 上層），**不是抖動**（seed 固定＝確定性）：bot 的麵粉／牛乳補貨沒限定月玩家。修掉後變慢的最多 +18%，數字在計畫檔 CP10。
- **「看得到整批」只驗了 instance 數**：裝模機、烤箱、冷藏櫃的機身會蓋住站上那一盤（截圖實證），整批最看得清楚的是爐台／打蛋／攪拌的碗、裝飾台、以及盤子在站與站之間滑動的時候。
- **已上線（2026-09-24）**：使用者回「好 合併」，PR #30 squash merge，master＝`e8791ad`（`ls-remote` 相符）；Pages workflow 綠；線上 bundle `index-BeCaZI3X.js` 含「店面人氣」「點卡片加份數」「出爐！」；`npm run smoke:live` 對線上全過。
- **教訓**：固定 seed 的節奏量表是確定性的——改前後差超過門檻不能說成「抖動」，要找出是哪一個改動（這次是 bot 補貨沒限定月玩家，advisor 抓到）。
- **待使用者**：在手機上看畫面（線上網址）；升級變貴（每台 Lv1→2 一律 619，改前爐台 120、烤箱 400；3 小時做出的甜點 600–900→約 350 份）是「同等困難」的直接代價，要先講。已知下一道牆：農場約 3 小時養滿 60 隻，月玩家的收入第 4 天起持平在 23–32 萬／天——長線靠的是離線累積＋機器／人氣兩條升級，農場本身沒有長線。
...
- **PR #28 已開**（第一版 commit `aeb801c`）。
- **第二版（同日使用者追加）**：「2/4份 這種容量跟現況的數據文字小一點 同時顯示更小的字說明這是第幾lv的機器 跟機器名稱 機器下方的名牌就可以移除掉了」。做法：輸送帶側板上的 3D 名牌拿掉（`LabelAtlas` 只貼店招；`STATION_LABEL` 刪掉），名稱只剩標籤裡，所以**七台一律顯示標籤**：第一行名稱 7.5px＋`Lv2` 7px（320px 分兩行），第二行份數數字 10px（原 11.5）、空著寫 `0/上限`、沒買寫「未購買」整塊變淡；徽章 `box-sizing:border-box` 整顆移到外緣上方（原本白邊蓋住「打蛋機 Lv2」的最後一個字，e2e 用 Range 量字的框抓到）。字縮小後 320px 也塞得下「份」，原本的窄版省字規則刪掉（負向對照不再紅＝規則多餘）。
...
> 不是每個甜點都需要五個步驟 你需要參考真實甜點食譜 而去一份甜點會需要多個原料
> 我希望製作甜點可以選擇要製作哪一種（想是閱覽菜單可以看的使用原料、總時長、失敗機率、販售價錢）
> 然後當你都有原料跟那些機器時 食譜會可以點按（其他缺少設備或原料要在個別菜單下面提示）
> 流水線放上去後就會自動完成所有甜點（但每個機器都有 只有上架時需要玩家參與）
> 前期機器可以製作的甜點份數比較少
...
### 使用者回「都照你的建議 開工」（2026-09-24）之後
- **平衡**：初版（整條線 430、升級 ×8、售價 ×2.2）中後期慢一倍 → 調成整條線 250（Lv1：爐台 20／打蛋 20／攪拌 30／裝模 40／烤箱 80／裝飾 60；冷藏 150）、Lv2 約 Lv1 的 4 倍、`dessertMarkup` 2.8。`npm run pacing`：上層 5.9–17.6、下層 25–36、二號 40–52 分，3h 金幣 33–40k（只賣原料 20.6k），D24 全達標。量表 bot 改成「解鎖上層後把焦糖布丁塔那條線升 Lv2、冷藏櫃等上層解鎖再買」（第一版見錢就買冷藏櫃＋補貨合約，量到的是 bot 花錢方式不是平衡）。**同一組價錢跑三次，上層在 18.4／21.1／20.5 之間跳——抖動比單一價格調整的效果大，判斷要看多情境、留餘裕**。
- **確認改讀**：「另外買自動化機」維持由「機器分級」取代。
- **工坊入口**：錢夠買焦糖布丁塔整條線、一台機器都沒有 → 「甜點店」鈕 `.alert` 掛珊瑚色「!」（e2e＋負向對照紅過）。
- **名牌放大**：0.46×0.13、字級自動縮到不超出牌子；iPhone SE 讀得到。
...
**使用者原話（逐字保留）**：
> 我現在要改變撿原料到賣甜點的過程
> 製作甜點有一個專屬的地方 你需要重新建模並且精緻可愛（參考主流甜點製作遊戲）
> 將布丁掉落的原料製成甜點的每個步驟都要有動畫跟對應的機器 我之後才能上架自動化機器讓玩家購買
> 而且烘焙甜點需要時間跟擺在架上販賣有甜點店開張的問題（營業時間、日營收）
> 所有現在布丁掉落的原料變成直接賣給商店，不再製作甜點；
> 各種口味的布丁也可以販售，請幫我訂出合理價格
> 另外我希望增加成就系統，讓一開始遊玩獲得初始資金，因為甜點製作自動化系統、加速、店員僱員 都應該很貴

**使用者四題選定（AskUserQuestion，2026-09-23）**：
1. 原料去向＝**兩條路都有**：掉落原料可直接賣給商店換小錢，也可送進甜點工坊走完整步驟做成甜點賣高價；農場上的一鍵「加工」拿掉，改由工坊負責。
2. 工坊位置＝**獨立場景切換**（畫面下方「甜點店」鈕切到另一個場景：工作檯＋各台機器、展示櫃與店門）。
3. 營業時間＝**遊戲內一天**（約 20 分鐘真實時間；白天營業、客人從展示架買走，晚上打烊結算當日營收；離線照規則補算）。
4. 製作步驟＝**五步**：打蛋 → 攪拌 → 裝模 → 烘烤（要等時間）→ 裝飾上架，每步一台機器＋動畫；之後每台都能上架成自動化設備。

**授權**：使用者說「上述完成後直接 merge」——**只涵蓋這一包（甜點工坊＋賣原料／賣布丁＋成就）**，不延伸到其他分支。merge 授權不等於視覺簽核：截圖與線上網址仍要交給使用者看。

**追加兩題（同日）**：自動販售口＝**退款拆掉**（跟加工機一樣照原價退）；訂單卡＝**搬進甜點店**（用工坊的成品交）。

### 做了什麼（D50–D54，計畫檔 CP8）
- 規則：`src/game/bakery.ts`（五站、營業日、客人、預訂單、上架；自動化旗標 `bakery.auto` 規則先做好、**商店未上架**）、`src/game/achievements.ts`（20 條）、`actions.sellPudding`、`SCHEMA_VERSION` 7→**8**（epoch＝升級當下的 time、退款含倉庫台數、`desserts` 留作成品櫃、`speciesSeen` 從住客補）。`EquipmentId` 只剩三台；`RETIRED_EQUIPMENT_PRICE` 給 migrate 退款。
- 場景：`src/scene/bakery/`（`room.ts` 靜態房間＋五台機身併一個 mesh、`bakeryView.ts` 動畫：打蛋殼裂開、攪拌頭轉＋麵糊變色、注模嘴來回＋杯子填滿、烤箱窗發光＋布丁膨起、轉台＋擠花、一盤在站間飛、展示櫃、成品櫃、小兔客人進出、營業牌、晝夜）。工坊 draw calls 實測 **20**（預算 35）。
- UI：「甜點店／回農場」切換、五站磚（倒數＋進度條、做完變綠）、日曆列（第幾天／時間／營業中／今日／昨日營收）、開工選單、預訂單鈕＋卡片、成就鈕＋卡片、商店賣布丁、工坊專用提示 `bakeryHint`。
- **踩過的坑（都靠截圖抓到）**：①鏡頭只照寬度算，後排工作檯被頂出畫面 → 改成投影 8 角框進 HUD 可見段；②ACES 把粉彩壓成灰白、降燈光也救不回 → 工坊材質 `toneMapped:false`，燈光按「亮度≈反照率×光強/π」重算（第一次調暗調過頭成一片泥色，就是忘了 /π）；③展示櫃實心頂板從上往下看把甜點整片蓋掉；④吊燈電線俯視是黑棍子；⑤門往內開擋客人路線；⑥預訂單卡疊在畫面上蓋掉半間店；⑦320px 頂列四個數字＋兩顆鈕塞不下（e2e 量到重疊 20px）。
- **截圖腳本的假警報**：`step()` 推得比 HUD 的 160ms 節流快，截到的是舊數字（看起來像 state 被重設）；`tools/shoot-bakery.mjs` 截圖前等 450ms。另外改完檔立刻截圖會撞到 Vite 重新載入。

### 證據（2026-09-23，worktree `../lpg-wt-bakery`）
- vitest **218 passed**（新 `bakery.test.ts` 33 條）；`npm run build` 綠；`npm run test:negative` 17 條中 16 條會紅——新的 AC8-1〜8 七條全紅過，**唯一不紅的 AC2-9 是 master 上本來就壞的那條**（測試名稱還寫 5 份，`dropCap` 早改 8）。
- e2e：第一輪 59 passed／6 failed（全是測試寫死舊機制：訂單在農場、設備頁三台捲不動、320px 頂列、撿完瞬間又掉一顆的時機），修完重跑見下。新 `cp8-bakery.spec.ts` 7 條（真的點按鈕走完五站→上架→客人買、點 3D 烤箱推站、預訂單卡交貨、賣布丁畫面少一隻、領成就、工坊 draw calls）。**e2e 突變兩組都紅過**：拿掉 `puddingSold` 的場景釋放 → 骨架 3≠2 紅；拿掉 320px 藏原料數的樣式 → 工坊那條紅。
- `smoke:live` 對本機正式建置 15 項全過（draw 19/35、SW、離線）；工坊在正式建置下截圖 draw 20、零錯誤。
- 簽核截圖在 session scratchpad（`shots/`、`shots-se/`、`shots-prod/`），**使用者尚未看**。

### 節奏（`npm run pacing` 改版前後，3 小時）——**這幾個平衡點要交給使用者定**
| 情境 | 上層（前→後） | 下層 | 二號櫥窗 | 3h 金幣 |
|---|---|---|---|---|
| equip-first seed 7 | 12.7 → **8.4** 分 | 22.2 → 17.3 | 39.5 → 41.3 | 62k → **35.5k** |
| zone-first seed 7 | 4.5 → 2.5 | 15.6 → 12.0 | 31.3 → 35.5 | 66k → 39k |
| equip-first seed 99 | 9.9 → **2.6** | 18.1 → 13.8 | 32.9 → 34.0 | 70k → 37.7k |
| hybrid seed 7 | 10.4 → **2.6** | 19.1 → 13.8 | 31.1 → 35.0 | 73k → 37.8k |
| raw-only（新：完全不進工坊） | — → 11.7 | 32.8 | 74.5 | 18.2k |
- 開局資金（前七條成就 640 元）約第 2.6〜8.5 分領齊；**三個情境因此 2.6 分就買得起上層（500）**，比改版前快很多。D24 目標（≤20／≤45／≤90 分）全達標。
- 3 小時總收入掉一半：舊的販售口把甜點按 4 倍原料價秒賣，現在是一條手動流水線＋客人上限（約 340 位客人、170 位撲空）——這正是「自動化／加速／店員很貴、值得買」的空間，但**自動化目前沒上架**，所以中後期偏慢。
- 工坊值得走：跑工坊 35.5k vs 只賣原料 18.2k。

### 已上線（2026-09-23）
- PR #22 squash merge，master＝`f8d474f`；`git ls-remote` 比對相符、Pages workflow 綠。線上 bundle `index-CmSBKzc2.js` 含只有新版才有的字串（小布丁甜點店／開一盤甜點／預訂單／混種收藏家／回農場）全部命中；`npm run smoke:live` 對線上全過。e2e 最後一輪 **67 passed / 0 failed**。
- 截圖已用 SendUserFile 傳給使用者，**視覺簽核尚未回覆**。

### 還沒做／待使用者
- 視覺簽核（工坊整體、機器造型、客人、按鈕配置）；上面三個平衡點（開局資金讓上層 2.6 分就買得起、3h 收入減半、自動化何時上架）。
- 工坊自動化五台（1500–5000）、烤箱加速（3000／8000）、店員（6000＋日薪 100）**只定價未上架**（使用者：「我之後才能上架」）。
- 新手引導在工坊有自己的一套（`bakeryHint`），**農場引導沒有講到成就以外的工坊細節**，實機玩一次再看要不要補。
...
## 販售動畫與提示框兩顆鈕（2026-09-22 第十場後半，使用者追加）

- **金幣演出（D42，`src/scene/coinsView.ts`）**：賣出→外帶窗口冒一下→金幣拋出→落地彈一下躺平→停 0.55 秒→縮小消失，全程約 2.3 秒。自己的 `InstancedMesh`（+1 draw call），**沒金幣在飛時 `visible = false`**。實測 draw 25–27（預算 35）。
- **調參數是靠截圖調出來的，不是算出來的**（兩輪都踩到，記法）：①`R = 0.032` 在 iPhone 視口下只有十幾 px，看起來像雜訊不像金幣 → 改 `0.05`（跟掉落原料 0.055 同量級）；②金幣落在外帶窗（`z = 0.68` 的 0.3×0.2 箱體）正後方 `z≈0.5` 會被它整個擋住，只看得到一角 → 加大 `vy`/`-vz` 拋過窗體落到 `z≈0.35` 的空地。
- **`?pause=1` 截動畫要小步推**：`window.__lpg.step(1.0)` 會把一整秒塞進一幀，`coins.update(1.0)` 直接把整段演出跑完，截到的是殘影。用 `step(0.05)` 連推才看得到過程。
- **提示框（D43）**：綠名牌「小布丁」→「小提示」；× 改成**只收起這一則**（不進 localStorage）、另加「不再顯示提示」才是永久。**警告類（stalled／zonefull）被 × 收起只安靜 45 秒**（`Hud.WARNING_SNOOZE_MS`）——不這樣做等於從另一扇門把 D39 修好的洞打開。
- **× 的 `aria-label` 不可以含「關閉」**：`getByRole('button', { name: '關閉' })` 是**子字串比對**，會跟商店的「關閉」鈕撞成 strict mode violation（`cp3-tap-tier`／`cp3-expansion` 因此紅）。現在是「收起這則提示」。
...
- **e2e 全跑 47/48**：紅的是 `cp3-game-loop.spec.ts:114`（AC3-1b 牛乳澡繁殖，卡在 `:139` 等 `panna` 基因 180 秒逾時），**不是** STATE 記過並修掉的 `:30`（AC3-1）。單跑 3/3 綠（13.9 秒）。**第二輪全跑 48/49**：`:114` 綠了，換 `cp5-pwa.spec.ts:82`（提示框 × 分工，`toHaveAttribute` 失敗）紅，單跑 5/5 綠。兩輪紅的不是同一條、都與這次改動無關（沒碰 `src/game/`／`hud.ts`），是全跑時機器忙的機率型失敗，但**兩條都未查明根因**。
...
- **PR #16 已 squash merge 上線（2026-09-23，merge commit `d23fb92`）**：`ls-remote` sha 相符、Pages deploy 綠，線上 bundle `index-BKl-tSm8.js` 含 `EquipmentDrag`／`lpg.hints.closed`／「擺到這一區」，`npm run smoke:live` 全過（draw 19/35）。worktree `../lpg-wt-storage` 已清（先 `cmd /c rmdir node_modules`）。

## 家具擺放模式＋倉庫格子（2026-09-23，D49 第二版，分支 `feat/storage-drop`，worktree `../lpg-wt-storage2`）

- **使用者看了第一版的回饋**（手機截圖）：①倉庫卡不要列「擺在上層的」——複數同名家具分不出收哪個櫥窗的，東西都在櫥窗時只顯示「倉庫是空的」；②倉庫裡要有圖片；③拿出來要**預設可以移動**；④「先思考一般遊戲怎麼做」。
- **照 Hay Day／動森的擺放模式重做**（第一版「放手就寫入」＋卡片上「收起來」鈕整套換掉；中途想過的「拖到下方放置區收起來」沒上線就作廢）：長按＝拿起來 → 放手仍在擺放模式、動作列換成「收進倉庫／取消／確定」（`.editbar`，擺放中 `.hud.editing` 把動作列／倉庫鈕／小提示藏起來）→ 按確定才 `moveFurniture`／`placeFromStorage`。擺放模式中手指在畫面任何地方拖都是拖這件（不必按準），鏡頭鎖住。紅圈時確定是 disabled。
- **倉庫卡**：`.sgrid` 一列三格，圖用商店同一套（`shopArt.furnitureArt`，設備＝`eq*.svg`、澡盆＝浴缸，特殊盆角落疊液體；同種多台有 ×N 角標），點了卡片收掉、直接進擺放模式（從倉庫來的沒有「收進倉庫」鈕，取消＝還在倉庫）。
- **規則層**：`storageError`（能不能拿出來）＋`storageSpot`（預設落點）＋`placeFromStorage(…, pos)`（位置必帶、不合法就擋、不自己挪）——預覽與寫入共用同一組判斷。從倉庫拿出來的盆 state 仍在倉庫，`BasinsView.sync` 的 preview 讓它照樣畫出來。
- **收進倉庫有液體**：`ui/storage.dumpWarning` → `.confirmcard`（倒掉並收起／取消），不用 `confirm()`。
- **注液閥**：拿起來只能收（提示改口、確定鈕藏起來）；`furnitureUnder` 改成依打到的 group 分類——注液閥就在澡盆正上方，只比水平距離會分不出按的是管子還是盆。
- **e2e 的機率性紅（本場踩到）**：「收進倉庫要確認 3 份」全跑紅、單跑綠——長按那 0.7 秒布丁跳進盆泡掉一份。測試裡先把布丁餵飽（`caramel = 100`），連跑三次 6/6。
- **證據**：單元 201 綠、e2e 61/61 綠（新流程 6 條；負向對照紅過：確定鈕永遠可按、點倉庫格子跳過擺放模式）、`npm run build` 綠、`layout.mjs` 兩個視口無疊、`test:negative` 只有 AC2-9（本來就紅）；截圖 iPhone 14／SE 看過（倉庫格子、擺放模式，第一次截圖發現按鈕列底下露出半截動作列 → 加 `.hud.editing` 藏起來）。
- **PR #18 已 squash merge 上線（2026-09-23，`ebeaff1`）**：`ls-remote` sha 相符、Pages deploy 綠，線上 bundle `index-CkrhK-KA.js` 含 `editbar`／「倒掉並收起」／「會擺到你正在看的」，`npm run smoke:live` 全過（draw 19/35）。worktree 已清。**視覺待使用者手機簽核**：倉庫格子、擺放模式按鈕列、長按手感。

## 「不再顯示提示」對警告沒作用（2026-09-23，使用者回報「不再顯示按鈕還是沒有作用」，分支 `fix/hint-off-all`）

- **對線上版實測重現**：「櫥窗住滿了」按了會消失（D49 修過），但「農場停住」（`stalled`）按「不再顯示提示」**只藏一幀、160ms 後 HUD 更新又冒出來**——D43 刻意讓警告無視這顆鈕，玩家看到的就是按了沒反應。先前的 e2e 只測過教學句與 ×，沒測過「不再顯示提示」碰到警告。
- 修法：「不再顯示提示」＝全部都不顯示（含警告）；× 的 45 秒冷卻不變；農場停住的事改由離線回來的歡迎卡繼續講。計畫 D43 補一句。
- 新 e2e（`cp5-pwa.spec.ts` 最後一條）：停擺 → 按「不再顯示提示」→ 等 1.5 秒仍隱藏 → 重新整理仍隱藏。**修之前跑紅過**。
- 證據：單元 201 綠、`npm run build` 綠、e2e 全跑 60 綠＋1 紅＋1 沒跑：紅的是 `cp3-expansion` 抹茶澡盆（等突變逾時），單獨重跑 2/2 綠——跟第 167 行那類全跑時的隨機逾時同型，這次改動沒碰到它，未處理。
- **PR #20 已 squash merge 上線（2026-09-23，`e649145`）**：Pages deploy 綠、sha 相符；對線上版重跑重現腳本，住滿與停擺兩種提示按「不再顯示提示」1.5 秒後都隱藏；`npm run smoke:live` 全過。worktree 已清。
- **教訓**：一顆按鈕在某些狀態下「刻意沒作用」，對玩家來說就是壞掉。要保留例外就要把按鈕藏起來或改字，不能讓它照常顯示卻沒反應。
...
2. 形式＝**手動動作 ＋ 另外買的自動機**（照 D40 焦糖刷的模式），不是買了就全自動。
3. 手動介面＝**點動作列進入選取模式 → 點那一隻布丁**（不是一鍵對整區，玩家要選得出「我要養這隻成抹茶」）。
4. 自動機**分開三台**，按等級陸續上架：自動焦糖刷 80／Lv.1、自動撒粉機 300／Lv.3、自動淋醬機 300／Lv.5（價格等級直接沿用原本兩個特殊澡盆，節奏不動）。
5. 刷焦糖**要扣焦糖庫存**——不扣的話 D35「沒料就停產」與補貨合約的焦糖那半邊全部失去意義。
6. `autoFill` 改名**「自動注乳閥」**，`Basin.preferredLiquid` 整個拿掉（只剩一種液體就不可能黏錯口味，那正是 D39 那個 bug 的溫床）。
7. 商店補給品叫**「宇治抹茶粉」**，抹茶布丁掉的原料維持「抹茶粉罐」——兩者名字近但用途不同，文案要講清楚。

**兩條規則是 Claude 補的，還沒經使用者拍板**
- 泡牛乳澡**不再補 caramel**（現在 `caramelAfterBath: 60`）。不拿掉的話焦糖刷半廢、D35 的因果又斷一次。
- 裝了撒粉機的那一區會**持續**對所有布丁（含牛乳澡生下的子代）撒粉 → 單親複製＋`gameteShiftChance` 之下那一區**收斂成純抹茶**，而 `sakura`（抹茶＋草莓）這種需要兩個不同等位基因的混種在該區就配不出來。建議①接受（自動機＝量產純種的工具、混種在沒裝機的區手動配，但商品描述要明講），備案②「只撒指定的幾隻」③「只對成年布丁撒」。

**動手前一定要重新確認的三件事（都是當下狀態，會過期）**
- **D 編號**：2026-09-23 01:2x 重查，撞號已解——`origin/master` 是 D44 WebGL context lost／D45 設備逐區／D46 迷你販賣機（PR #8 標題誤寫 D45，檔案裡是 D46）；`feat/zone-capacity-15` 把物種外觀／表情改成 D47／D48。本改版取**動手當下**最大值 +1（照現況是 D49 起），落筆前重查 `grep -o "^| D[0-9]*" docs/plans/game-plan-v1.md`。
- **`SCHEMA_VERSION`**：不要預先寫死。`origin/master` 已是 6（設備逐區），本改版要取「動手當下讀到的值 +1」。
- **遷移補值方向**（補錯不會有測試紅）：`ownedBasins` 有 matcha／strawberry → **送對應自動機，裝在「那個盆實際所在的那一區」**（`Basin.zone`，不是起始區）；盆裡剩的抹茶／草莓／焦糖 **units 要退回 `stock`**；正在泡那些液體的布丁要退回 `resting` 並清 `basinIndex`／`bathLiquid`／盆的 `occupantId`。

**這一場的環境事故（跟上面第 95 行那次同型）**
同一個工作樹又有並行 session 在即時編輯：15:05 時 29 個檔未提交，正在做「設備每一區各買各的」（`state.equipment` → `Record<zoneId, Record<EquipmentId, boolean>>`、`SCHEMA_VERSION` 5→6），動的是 `state.ts`／`shop.ts`／`actions.ts`／`equipment.ts`／`ui/shop.ts`／`game-plan-v1.md`——**跟本改版要動的檔幾乎完全重疊，而且兩邊都要升存檔版本**。所以本場只寫計畫與續跑檔，沒碰 `src/`。另外本地 `master` 與 `origin/master` **分岔**（本地多 3 個未推的 WP7-1 commit，origin 多 `eaf96e4`），開 feature 分支前要先對齊，否則 PR 會夾帶那三個。
