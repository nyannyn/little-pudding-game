...
export async function hudBoxes(page) {
  return page.evaluate(() => {
    ...
    return { vh: innerHeight, vw: innerWidth, topbar: box('.topbar'), zones: box('.zones'), dock: box('.dock'), hint: box('.hint'), orders: col, cards, rows: [...document.querySelectorAll('.living .row')].map((el) => { const b = el.getBoundingClientRect(); return { l: b.left, t: b.top, r: b.right, b: b.bottom, text: el.textContent }; }) };
  });
}
