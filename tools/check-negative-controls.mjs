...
const CASES = [
  ...
  {
    ac: 'AC9-4b',
    why: '份數改回「照機器上限扣」，升到 Lv3 之後材料不夠 4 份就開不了工（花錢買降級）',
    file: 'src/game/bakery.ts',
    from: '  const qty = batchQty(state, species);',
    to: '  const qty = batchQty(state, species) > 0 ? Math.max(batchQty(state, species), 4) : 0;',
    test: '份數是上限不是門檻',
  },
  {
    ac: 'AC9-6b',
