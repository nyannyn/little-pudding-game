...
const CASES = [
  ...
  },
  {
    ...
    from: '  const coins = ingredientSellPrice(species, star) * n;',
    to: '  const coins = 0;',
    test: '撿起來就入庫，賣掉就加錢',
  },
  {
  ...
  // AC8-1／AC8-2（手動推站、開工只驗蛋）在 D56／D57 食譜制全自動流水線後不存在了，換成下面 AC9-*
  {
    ...
    from: 'const t = time - state.bakery.epoch +',
    to: 'const t = time +',
  ...
  },
  {
    ...
    // D64 起保留量在 `spareDesserts` 裡按星級扣；改壞＝不扣任何訂單
    from: '  for (const o of live) {\n    let need = o.qty;',
    to: '  for (const o of [] as typeof live) {\n    let need = o.qty;',
  ...
  },
  {
    ...
    file: 'src/game/recipes.ts',
    from: '  return Math.min(linePortions(state, species), affordablePortions(state, species, star));',
    to: '  return affordablePortions(state, species, star) >= linePortions(state, species) ? linePortions(state, species) : 0;',
    test: '份數是上限不是門檻',
  },
  // ── CP10（D60／D61）──
  {
  ...
  },
  {
    ...
    file: 'src/game/bakery.ts',
    from: '  if (hasStaff(state)) stockShelf(state, emit, true, wants);',
    to: '',
    test: '人氣 Lv6 起有店員',
  },
  {
  ...
  },
  {
    ...
    file: 'src/game/bakery.ts',
    from: "    if (id !== 'crack') addStock(state, 'ingredients', b.species as SpeciesId, 1, qty * LEGACY_ING_PER);",
    to: '',
    test: '打蛋站那盤只退蛋',
  },
  // ── CP11 星級布丁與常客（D62–D72，2026-09-25）──
  {
    ac: 'AC11-1a',
    why: '拿掉「只算精養區」，量產區 15 隻每隻都在升星，精養就沒有代價',
    file: 'src/game/stars.ts',
    from: '  if (!isEliteZone(state, p.zone)) return false;',
    to: '',
    test: '量產區泡幾次本命液照顧點數都是 0',
  },
  {
    ac: 'AC11-1b',
    why: '本命液只看第一個等位基因，混種泡第二種液體只拿 +1',
    file: 'src/game/stars.ts',
    from: '  return p.genes.some((a) => NATIVE_LIQUID[a] === liquid);',
    to: '  return NATIVE_LIQUID[p.genes[0]] === liquid;',
    test: '混種兩個等位基因的液體都算本命液',
  },
  {
    ac: 'AC11-2a',
    why: '精養區當溢出候選，寶寶被塞進精養區',
    file: 'src/game/breeding.ts',
    from: "    if (n === undefined || z.mode === 'elite' || n >= BALANCE.zoneCapacity) continue;",
    to: '    if (n === undefined || n >= BALANCE.zoneCapacity) continue;',
    test: '精養區有空位也不當落點',
  },
  {
    ac: 'AC11-2b',
    why: '母體住精養區、那區沒滿，寶寶就生在自己那區（鮮奶酪系會把精養區生滿）',
    file: 'src/game/breeding.ts',
    from: '  if (!isEliteZone(state, homeZone) && zoneHasRoom(state, homeZone)) return homeZone;',
    to: '  if (zoneHasRoom(state, homeZone)) return homeZone;',
    test: '精養區沒住滿，母體自己那一區也不收寶寶',
  },
  {
    ac: 'AC11-3a',
    why: '子代潛力寫死 2，世代鏈斷掉、永遠養不出 ★3 以上',
    file: 'src/game/breeding.ts',
    from: '    potential: childPotential(parent),',
    to: '    potential: 2,',
    test: '★3 母體 → 子代潛力 4',
  },
  {
    ac: 'AC11-3b',
    why: '風味突變時重設星級，玩家不敢讓精養布丁泡特殊澡盆',
    file: 'src/game/pudding.ts',
    from: '    applySpeciesAsPure(p, p.pendingMutation);',
    to: '    applySpeciesAsPure(p, p.pendingMutation);\n    p.star = 1;',
    test: '風味突變換物種但保留星級與潛力',
  },
  {
    ac: 'AC11-4a',
    why: 'hud.ts 繞過 stock.ts 直接索引庫存',
    file: 'src/ui/hud.ts',
    from: "  return totalStock(s, 'desserts');",
    to: "  void s.ingredients.caramel;\n  return totalStock(s, 'desserts');",
    test: '庫存單一出入口',
  },
  {
    ac: 'AC11-4b',
    why: 'tools/ 的試玩腳本直接改庫存（不在 tsconfig 裡，tsc 管不到，會把數字接到陣列後面）',
    file: 'tools/playtest/lib.mjs',
    from: '    const s = window.__lpg.state;',
    to: '    const s = window.__lpg.state;\n    s.ingredients.caramel += 1;',
    test: '庫存單一出入口',
  },
  {
    ac: 'AC11-5',
    why: '材料夠不夠改成全部星級加總，那一星不夠就從別的星級湊',
    file: 'src/game/recipes.ts',
    from: "  return stockOf(state, 'ingredients', key, star);",
    to: "  return stockOf(state, 'ingredients', key);",
    test: '那一星不夠就拒絕',
  },
  {
    ac: 'AC11-6a',
    why: '拿掉散客上限，高星甜點賣給誰都一樣、常客就只是另一種散客',
    file: 'src/game/stars.ts',
    from: '  return starMult(clampStar(Math.min(star, BALANCE.walkInStarCap)));',
    to: '  return starMult(star);',
    test: '散客買 ★4 只付 ★2 的價',
  },
  {
    ac: 'AC11-6b',
  ...
  {
    ac: 'AC11-10a',
    why: '舊庫存放到 ★5，老玩家一上線就拿到高星貨',
    file: 'src/game/stock.ts',
    from: "    if (typeof v === 'number') row[0] = cleanCount(v);",
    to: "    if (typeof v === 'number') row[4] = cleanCount(v);",
    test: '所有舊庫存在 ★1',
  },
  {
    ac: 'AC11-10b',
    why: '漏補掉落物的星級，撿起舊存檔地上的原料會落到 undefined 星',
    file: 'src/game/state.ts',
    from: '          star: clampStar(src.star),\n        };',
    to: '        };',
    test: '掉落物 ★1',
  },
];
